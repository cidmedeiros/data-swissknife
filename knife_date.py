...
def make_date(year,month,day):
    ...
    """
    Make the date out of seperated integer values. 
    """
    date = dt.date(year = year, month=month, day=day)
    
    return date

def make_date_series(series_year, series_month, series_day):
    
    """
    Applies make_date on a non date format pandas series object. Ideal for datasets with dates
    organized seperated columns for year, month and day, instead of single column for date.
    series_year: pandas series type int;
    series_month: pandas series type int;
    series_day: pandas series type int;
    
    """
    serie = pd.to_datetime((series_year*10000+series_month*100+series_day).apply(str),format='%Y%m%d')
    
    serie = [make_date(x.year, x.month, x.day) for x in serie]
